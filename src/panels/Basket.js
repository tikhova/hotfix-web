...
const Basket = ({match: {params: {areaId, itemId}}, foodAreas, order}) => {
    // FIX 5
    const options = JSON.parse(localStorage.getItem('basket_options') || null)
    const [faster, setFaster] = useState(options ? options.faster : true);
    const [time, setTime] = useState(options ? options.time : '');
    const [selfService, setSelfService] = useState(options ? options.selfService : false);
    //
        ...
        <div className="Place">
            <header className="Place__header">
                <aside className="Place__trz">
                    <h1 className="Place__head">
                        <Link to="/" className="Place__logo" onClick={() => {
                            localStorage.setItem('basket_options', null) // FIX 5
                        }}>
                            {area.name}
                        </Link>
                    </h1>
            ...
            <div className="Place__products-wrapper">
                    ...
                    {products.map(({item, count}) => (
                        ...
                        >
                            <img
                                ...
                                src={item.image}
                            />
                            <h3 className="Place__product-name">
                                {item.name}
                            </h3>
                ...
                <Link
                    ...
                    // FIX 5
                    onClick={() => {
                        localStorage.setItem('basket_options', JSON.stringify({
                            faster,
                            time,
                            selfService
                        }));
